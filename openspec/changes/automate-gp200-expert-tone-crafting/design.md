...
This change introduces both UI automation and expert tone-authoring logic. The automation side must tolerate editor-state drift, connection loss, and unsaved patch state. The authoring side must convert tone intent into concrete GP-200 module choices and parameter values while respecting routing context such as `FRFR`, `headphones`, `interface`, `amp front`, `amp return`, and `4CM`.
...
**Goals:**
- Provide a deterministic control layer for `GP-200 Edit` on Windows.
...
- Make automation safe by detecting connection state, unexpected screens, and unsaved edits before taking destructive actions.
...
**Non-Goals:**
- Reverse engineer the GP-200 USB/MIDI protocol or bypass the editor with undocumented device control.
...
- A `session/controller` layer that launches `GP-200 Edit`, confirms the device/editor is ready, and exposes safe editor actions.
...
Before mutating the editor, the automation must verify:
- The software is running and foregroundable
- A GP-200 connection is present
...
- [Device disconnects during editing] -> Mitigation: add connection health checks before patch mutations and before save operations.
...
- Which Windows automation surface is most reliable for `GP-200 Edit` in this environment: UI Automation, image-based automation, or a hybrid?
- How much parameter readback is possible from the editor UI versus requiring trust in the last applied action?
- Should the first save workflow write directly into user-selected patches, duplicate to a safe destination, or export backups before mutation?
